...
## 7. 工程计算案例：爬电距离与高压绝缘设计

### 7.1 爬电距离基本概念

**爬电距离（Creepage Distance）**：沿绝缘表面测得的两个导电部件之间的最短路径。防止潮湿/脏污环境下形成导电路径导致漏电或电弧。

**核心公式**：

```
最小标称爬电距离 lt = 应用系数(a) × 最小标称爬电比距(lf) × 额定电压(Ur) × 直径校正系数(kD)
```
...
| 变量 | 说明 |
|------|------|
| 系统最高工作电压 | 电压越高，所需爬电距离越长 |
| 绝缘材料 CTI 值 | CTI 越高抗爬电性能越好，所需距离越短 |
| 环境污染等级 | 等级越高（越脏污），所需距离越长 |

### 7.2 直流 800V 系统爬电距离估算

| 污染等级 | 交流爬电比距 (mm/kV) | 直流系数 | 直流爬电比距 (mm/kV) | 800V 估算距离 |
|-----------|---------------------|---------|---------------------|--------------|
| Ⅱ级（中等）| 20 | 1.2 | 24 | ~19 mm |
| Ⅲ级（重污秽）| 25 | 1.5 | 37.5 | ~30 mm |

> **PCB 设计参考**：500V 下 FR4（CTI~175-250）最小爬电距离 ≥6.4mm；800V 系统污染等级 2~3 时需 ≥9.6mm。

### 7.3 设计关键考虑

1. **电气间隙与爬电距离需同时满足**：爬电距离在任何时候都不应小于电气间隙
2. **应用系数**：相与地=1，相与相=√3（需更大距离）
3. **材料与结构优化**：开槽增加表面路径、使用高 CTI 板材、表面涂覆高性能绝缘胶
4. **遵循产品标准**：GB 19517（低压电器）、IEC 60664-1、IEC 60079-11（防爆）

### 7.4 案例：CZT 探测器 -800V 高压打火分析

**现象**：四面拼 CZT 探测器在 -800V 高压下出现打火声并内部短路，外部无明显击穿点。

**可能原因**：

| 原因类别 | 具体分析 |
|---------|---------|
| 晶体内部缺陷 | 杂质/位错/沉积物成为电荷陷阱，高压下形成局部导电通道 |
| 电极接触不良 | 欧姆接触/Chip 接触制备不佳，局部电场集中引发微放电 |
| 拼接界面风险 | 绝缘材料不足/气泡/污染物，爬电距离设计不足 |
| 外部高压耦合 | 高压电缆绝缘损坏，电脉冲耦合到探测器导致过压击穿 |

**排查与改进建议**：

- 精密外观检查（高倍显微镜找烧蚀点/金属迁移痕迹）
- I-V 特性曲线测试（判断接触质量/体内缺陷）
- 优化电极工艺（精确控制沉积/退火参数）
- 强化绝缘设计（增加有效爬电距离、表面涂覆绝缘保护胶）
- 严格环境控制（超净组装，防止污染物附着）
- 阶梯升压老化测试（低压下发现绝缘薄弱点）
